...
| 1 | Las agencias tienen tipos de precio (fidelidad, rack…) | **Decisión de modelo, y tenía un bloqueador debajo que nadie había visto.** `registros` guardaba el nombre de la agencia **en texto** y nunca llenaba `agencia_id`: la reserva no sabía con qué agencia trataba, así que no había de dónde colgar una tarifa. También explicaba dos cosas vivas: la cartera agrupaba deudores por cómo se escribiera el nombre, y las comisiones de la 026 —que se liquidan por `organizacion_id`— no alcanzaban a ninguna reserva. **Arreglado** (front + migración 034 de backfill). Falta el modelo: `tarifas` por plan × tipo × temporada, con la regla 4 intacta | Migración 034 ✅ · el modelo de tipos, pendiente de la lista real |
...
|---|---|---|---|
| **A1** ✅ **hecho** | Pegar el WhatsApp → reserva prellenada | El mensaje se pega tal cual y se lee fecha («mañana», «el sábado», «15 de agosto», «15/08»), cuántos («4 pax», «(x4)», «2 adultos y 1 niño»), plan, agencia y titular. **Vista previa antes de aplicar**: se ve qué entendió antes de que toque el formulario. Lo que no se entiende no aparece — un campo vacío cuesta un clic, uno mal deducido cuesta una reserva al precio equivocado | Medio |
| **A2** ✅ **hecho** | La agencia primero, con memoria | Al elegir agencia se propone lo que **suele pedir** —canal, plan, forma de pago, tipo— sacado de sus últimas 10 reservas, solo con mayoría clara (≥2 y más de la mitad). No pisa nada de lo ya escrito y trae «Deshacer». La tarifa entra cuando exista el modelo de tipos | Bajo |
